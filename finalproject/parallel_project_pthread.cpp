...
pthread_mutex_t task_lock[num_of_tasks][16];
...
int main(){
    pthread_t *ThreadId = (pthread_t*)malloc(num_thread * sizeof(pthread_t));
    ...
    int *data = (int*)malloc(sizeof(int) * num_thread);
    ...
    for (int i = 0; i < num_thread; i++)
    {
        pthread_create(&ThreadId[i], NULL, Work, (void*) &data[i]);
    }
    for (int i = 0; i < num_thread; i++)
    {
            ...
            for(int worker = tid * (num_of_workers / num_thread) ; worker < (tid + 1) * (num_of_workers / num_thread) ; ++worker)
            {
                ...
                if(local_load > 0)
                {
                    ...
                    if(local_load <= 0)
                    {
                        ...
                        pthread_mutex_lock(&task_lock[receipt][0]);
                        ...
                        pthread_mutex_unlock(&task_lock[receipt][0]);

                        pthread_mutex_lock(&task_lock[receipt][1]);
                        if(T[receipt].Left > 0)
                        {
                        ...
                        pthread_mutex_unlock(&task_lock[receipt][1]);
                    }
                ...
                if(local_load <= 0)
                {
                    ...
                    for(int select_task_id = 0; select_task_id < num_of_tasks; ++select_task_id)
                    {
                        ...
                        pthread_mutex_lock(&task_lock[s][1]);
                        ...
                        pthread_mutex_unlock(&task_lock[s][1]);
                    }
